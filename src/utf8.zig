...
//! `utf8Valid(bytes) -> bool` primitive that the generator calls
...
//!
//!   * **ON**  — `utf8Valid` runs a real validator and the encoder rejects a
//!     non-UTF-8 `string` with `error.InvalidArgument`.
//!   * **OFF** — `utf8Valid` folds to `return true` (zero validation cost, no
...
test "valid: empty, ASCII, embedded NUL, and multibyte code points" {
    try testing.expect(utf8Valid(""));
    try testing.expect(utf8Valid("hello, world"));
    // Embedded U+0000 is valid UTF-8 and MUST be accepted (CORELIB_PLAN §6.4).
    try testing.expect(utf8Valid("a\x00b"));
    try testing.expect(utf8Valid(&[_]u8{0x00}));
    // 2-byte (¢ U+00A2), 3-byte (€ U+20AC), 4-byte (𐍈 U+10348), and the
    // maximum scalar U+10FFFF (F4 8F BF BF).
    try testing.expect(utf8Valid("\xC2\xA2"));
    try testing.expect(utf8Valid("\xE2\x82\xAC"));
    try testing.expect(utf8Valid("\xF0\x90\x8D\x88"));
    try testing.expect(utf8Valid("\xF4\x8F\xBF\xBF"));
}
...
test "invalid: overlong encodings (including C0 80)" {
    ...
    try testing.expect(!utf8Valid(&[_]u8{ 0xC0, 0xAF }));
    try testing.expect(!utf8Valid(&[_]u8{ 0xE0, 0x80, 0xAF }));
    // Overlong U+0000 as 4 bytes (F0 80 80 80).
    try testing.expect(!utf8Valid(&[_]u8{ 0xF0, 0x80, 0x80, 0x80 }));
}

test "invalid: surrogate code points U+D800..U+DFFF" {
    if (comptime !STRICT_UTF8) return;
    // U+D800 (lone high surrogate) encoded as ED A0 80.
    try testing.expect(!utf8Valid(&[_]u8{ 0xED, 0xA0, 0x80 }));
    // U+DFFF (low surrogate) encoded as ED BF BF.
    try testing.expect(!utf8Valid(&[_]u8{ 0xED, 0xBF, 0xBF }));
}

test "invalid: code points above U+10FFFF" {
    if (comptime !STRICT_UTF8) return;
    // U+110000 encoded as F4 90 80 80 — the first scalar past the Unicode max.
    try testing.expect(!utf8Valid(&[_]u8{ 0xF4, 0x90, 0x80, 0x80 }));
    // F5.. and FF are never valid lead bytes.
    try testing.expect(!utf8Valid(&[_]u8{ 0xF5, 0x80, 0x80, 0x80 }));
...
test "invalid: truncated multibyte and bare continuation byte" {
    ...
    try testing.expect(!utf8Valid(&[_]u8{ 0xE2, 0x82 }));
    // 2-byte lead with no continuation.
    try testing.expect(!utf8Valid(&[_]u8{0xC2}));
    // Bare continuation byte with no lead.
    try testing.expect(!utf8Valid(&[_]u8{0x80}));
    // Valid text followed by a stray continuation byte.
    try testing.expect(!utf8Valid("ok\x80"));
}

test "strict-off build folds utf8Valid to accept-verbatim" {
